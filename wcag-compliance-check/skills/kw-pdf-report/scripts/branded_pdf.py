...
def _metadata_block(metadata):
    ...
    rows = [
        [
            Paragraph(str(label).upper(), _meta_label_style),
            Paragraph(str(value), _meta_value_style),
        ]
...
def _badge_row(fit_tier=None, operational_status=None):
    ...
    if fit_tier:
        labels.append(
            FIT_TIER_LABELS.get(
                fit_tier.strip().lower(), "FIT TIER: %s" % fit_tier.upper()
            )
        )
    ...
    if operational_status:
        ...
        labels.append(
            OPERATIONAL_STATUS_LABELS.get(
                key, "STATUS: %s" % operational_status.upper()
            )
        )
