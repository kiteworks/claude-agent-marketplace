...
Restores instructions that were cut off mid-sentence: the content-aware pass for PDF, DOCX, PPTX and XLSX files is now fully described, and the agent's summary card states which pass produced each proposed destination — filename-only or content-aware — so you can see how much to trust each suggestion.
