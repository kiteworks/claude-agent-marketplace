...
# surface-gate â€” verify capabilities before choosing a path
...
## Step 1 â€” verify and recover access where supported

- Subagent isolation: use the named subagent if delegation is available. Missing
...
## Step 2 â€” respond according to what the task needs

**Tier A â€” full support.** Required mechanisms have been verified, including
isolation when requested. Proceed, preserving the binary scratch disclosure.

**Tier B â€” degrade with disclosure.** Subagent isolation is absent but the actual
task mechanisms are available. Before doing anything else in the response, as
...
> so I'm running it directly in this conversation instead â€” its restrictions are
...
**Tier C â€” unavailable after recovery.** State the actual missing or failed
